...
const WINDOWS_OS_LABEL = "Windows";
...
export function describeAddProjectError(message: string): string | null {
  if (isDuplicateProjectCreateError(message)) {
    return `这通常表示该文件夹已关联到现有项目。在 ${WINDOWS_OS_LABEL} 上，同一文件夹可能以不同路径格式出现，因此看起来像是新项目，实则不然。`;
  }
  ...
  if (
    ...
    return "这是从文件系统根目录开始的绝对路径。如果文件夹在你的主目录中，请使用 ~/Developer/... 或完整的 /Users/<name>/Developer/... 路径。";
  }
